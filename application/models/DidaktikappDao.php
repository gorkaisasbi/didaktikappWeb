...
class DidaktikappDao extends CI_Model{
    ...
    function getUser($username){
        ...
        if(is_object($usuario)){
            $sql="SELECT g.idgrupo, nombregrupo, fechainicio, fechafin FROM grupos g WHERE g.idgrupo in (SELECT d.idgrupo FROM cardGrupos d WHERE d.idgrupo = g.idgrupo AND idusuario = $usuario->idusuario)";
            $rs = $this->db->query($sql);
    ...
    function getListas($idusuario){
        ...
        foreach ($listas as $lista) {
            $sql="SELECT g.idgrupo,nombreGrupo FROM listasGrupos l , grupos g WHERE g.idGrupo = l.idGrupo AND l.idLista = $lista->idlista";
            $rs = $this->db->query($sql);
    ...
    function buscarGrupos($nombreGrupo,$fechaHasta,$horaHasta,$fechaDesde,$horaDesde,$chechListas,$idusuario){
        ...
        $sql = "SELECT idgrupo,nombreGrupo FROM grupos g $where ORDER BY nombreGrupo LIMIT 10";
        $rs = $this->db->query($sql);
        $grupos = $rs->result();
        return $grupos;
    ...
    function getAct1($idAct1=-1){
        $sql = "SELECT id, estado, fragment, foto1, foto2, sopa FROM actividadZumeltzegi WHERE id = $idAct1";
        $rs = $this->db->query($sql);
        $act1 = $rs->row();
        return $act1;
    }
    ...
    function getAct2($idAct2=-1){
        $sql = "SELECT id, estado, fragment, test, foto1 FROM actividadSanMiguel WHERE id = $idAct2";
        $rs = $this->db->query($sql);
        return $rs->row();
    }

    function getAct3($idAct3=-1){
        $sql = "SELECT id, estado, fragment, test, foto1, foto2, foto3 FROM actividadUnibertsitatea WHERE id = $idAct3";
        $rs = $this->db->query($sql);
        return $rs->row();
    }

    function getAct4($idAct4=-1){
        $sql = "SELECT id, estado, fragment, puzle, palabra FROM actividadTrena WHERE id = $idAct4";
        $rs = $this->db->query($sql);
        return $rs->row();
    }

    function getAct5($idAct5=-1){
        $sql = "SELECT id, estado, fragment, frases, foto1, foto2 FROM actividadErrota WHERE id = $idAct5";
        $rs = $this->db->query($sql);
        return $rs->row();
    }

    function getAct6($idAct6=-1){
        $sql = "SELECT id, estado, fragment, test, puzle FROM actividadGernika WHERE id = $idAct6";
        $rs = $this->db->query($sql);
        return $rs->row();
    }

    function buscarGruposAdd($nombreGrupo,$idusuario){
        $sql = "SELECT idgrupo,nombreGrupo FROM grupos g WHERE nombreGrupo LIKE '%$nombreGrupo%' AND 0< (SELECT COUNT(*) FROM listasGrupos l WHERE g.idGrupo = l.idGrupo AND l.idlista in (SELECT idlista FROM listas WHERE idusuario = $idusuario) ) AND 0 = (SELECT COUNT(idgrupo) FROM cardGrupos c WHERE c.idgrupo = g.idgrupo) ORDER BY nombreGrupo LIMIT 10";
        $rs = $this->db->query($sql);
        $grupos = $rs->result();
        return $grupos;
